...
class ObjectDetectionProcessor:
    ...
    def process_half_frame(self, image_np):
            ...
            for i in range(num_detections):
                ...
                if (score >= .3 and self.category_index[class_id]["name"] in ['Hardhat','Helmet','Safety Vest','Vest']) or (self.category_index[class_id]["name"]=='Goggle' and score >= .3):
                    y_min, x_min, y_max, x_max = box
                    y_min = int(y_min * image_np.shape[0])
                    x_min = int(x_min * image_np.shape[1])
                    y_max = int(y_max * image_np.shape[0])
    ...
    def process_video(self, output_path, input_path=0):
        ...
        out = cv2.VideoWriter(f'outpy{ts}.avi', int(current_fourcc), 20, (frame_width, frame_height))
