...
class ObjectDetectionProcessor:
    def __init__(self, model_dir, labels_path, min_conf_thresh=0.5):
        ...
        # shoe_model_path = '/home/cognitica-i7-13thgen/NPS/Tensorflow-model-inference/fine_tuned_model/content/fine_tuned_model/saved_model'
        shoe_labels_path = '/home/cognitica-i7-13thgen/NPS/Tensorflow-model-inference/shoe_label.pbtxt'
        # shoe_labels_path = '/home/cognitica-i7-13thgen/NPS/Tensorflow-model-inference/ppe_label_map.pbtxt'
    ...
    def process_half_frame(self, image_np):
            ...
            for i in range(num_detections):
                ...
                if (score >= .5 and self.category_index[class_id]["name"] in ['Hardhat','Helmet','Safety Vest','Vest']) or (self.category_index[class_id]["name"]=='Goggle' and score >= .4):
                    y_min, x_min, y_max, x_max = box
                    y_min = int(y_min * image_np.shape[0])
                    x_min = int(x_min * image_np.shape[1])
                    y_max = int(y_max * image_np.shape[0])
    ...
    def process_bottom_half_frame(self, image_np):
            ...
            for i in range(num_detections):
                ...
                class_id = detections['detection_classes'][i]
                score = detections['detection_scores'][i]
                print('name : ',self.category_index[class_id]["name"] )
    ...
    def process_top_half(self, roi_person):
        top_half = roi_person[roi_person.shape[0]//6:roi_person.shape[0]//2, :] #final
        # top_half = roi_person[:roi_person.shape[0]//2, :] 
    ...
    def process_bottom_half(self, roi_person):
        bottom_half = roi_person[roi_person.shape[0]//3:, :]
        height, width, _ = bottom_half.shape
        second_quarter = bottom_half[height//2:, :]

        # Display the quarters if needed
        cv2.imshow('Second Quarter', cv2.resize(second_quarter, (640, 240)))
    ...
    def process_video(self, output_path, input_path=0):
        # cap = cv2.VideoCapture('/home/cognitica-i7-13thgen/NPS/camera_videoRec/outpy2024-03-12 11:19:45.595579.avi')
        cap = cv2.VideoCapture('/home/cognitica-i7-13thgen/NPS/camera_videoRec/outpy2024-03-12 13:52:17.689262.avi')
